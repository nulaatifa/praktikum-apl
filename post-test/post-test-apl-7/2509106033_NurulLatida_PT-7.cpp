...
int  binarySearchHarga(Film *array_film, int jumlah, int harga_dicari);
void linearSearchKata(Film *array_film, int jumlah, string genre_dicari);
...
int binarySearchHarga(Film *array_film, int jumlah, int harga_dicari) {
    int batas_kiri  = 0;
    int batas_kanan = jumlah - 1;
    while (batas_kiri <= batas_kanan) {
        int posisi_tengah = batas_kiri + (batas_kanan - batas_kiri) / 2;
        if ((array_film + posisi_tengah)->harga_sewa == harga_dicari) {
            return posisi_tengah;
        } else if ((array_film + posisi_tengah)->harga_sewa < harga_dicari) {
            batas_kiri = posisi_tengah + 1;
        } else {
            batas_kanan = posisi_tengah - 1;
        }
    }
    return -1;
}
...
void linearSearchKata(Film *array_film, int jumlah, string genre_dicari) {
    ...
    for (int i = 0; i < jumlah; i++) {
        Film *penunjuk = array_film + i;
        if (penunjuk->genre_film == genre_dicari) {
            string status_film = penunjuk->tersedia ? "Tersedia" : "Disewa  ";
            string harga_str   = "Rp" + to_string(penunjuk->harga_sewa);
            cout << "  | " << left << setw(5)  << penunjuk->kode_film
                 << "| "           << setw(29) << penunjuk->judul_film
                 << "| "           << setw(10) << penunjuk->genre_film
                 << "| "           << setw(5)  << penunjuk->tahun_rilis
                 << " | "          << setw(8)  << harga_str
                 << "| "           << status_film << "|" << endl;
            ditemukan = true;
        }
    ...
    cout << "  +-------+------------------------------+-----------+-------+----------+---------+" << endl;
    if (!ditemukan) {
        cout << "  [!] Film dengan genre \"" << genre_dicari << "\" tidak ditemukan." << endl;
    }
}
...
void inputTanggal(int &hari, int &bulan, int &tahun) {
    ...
    while (!input_valid) {
        try {
            ...
            cin >> hari;
            validasiInputCin();
            validasiRentangInt(hari, 1, 31, "Hari");
            input_valid = true;
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
    ...
    while (!input_valid) {
        try {
            ...
            cin >> bulan;
            validasiInputCin();
            validasiRentangInt(bulan, 1, 12, "Bulan");
            input_valid = true;
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
    ...
    while (!input_valid) {
        try {
            ...
            cin >> tahun;
            validasiInputCin();
            validasiMinInt(tahun, 2000, "Tahun");
            input_valid = true;
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
...
void menuLihatFilm() {
    ...
    while (!pilihan_valid) {
        try {
            ...
            cin >> pilihan_sort;
            validasiInputCin();
            validasiRentangInt(pilihan_sort, 1, 3, "Pilihan");
            pilihan_valid = true;
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
    ...
    cout << "  1. Lihat detail film (by kode)"                      << endl;
    cout << "  2. Cari film by harga        [Binary Search]"        << endl;
    cout << "  3. Cari film by genre        [Linear Search]"        << endl;
    cout << "  0. Kembali"                                          << endl;
    ...
    while (!pilihan_valid) {
        try {
            ...
            cin >> pilihan_aksi;
            validasiInputCin();
            validasiRentangInt(pilihan_aksi, 0, 3, "Pilihan");
            pilihan_valid = true;
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
    ...
    if (pilihan_aksi == 1) {
        try {
            ...
            validasiTidakKosong(kode_detail, "Kode film");
            int indeks_detail = cariFilm(kode_detail);
            if (indeks_detail == -1) throw runtime_error("Film tidak ditemukan.");
    ...
    } else if (pilihan_aksi == 2) {
        Film salinan_film[MAKS_FILM];
        for (int i = 0; i < jumlah_film; i++) salinan_film[i] = daftar_film[i];
        for (int i = 1; i < jumlah_film; i++) {
            Film film_kunci = salinan_film[i];
            int  j          = i - 1;
            while (j >= 0 && salinan_film[j].harga_sewa > film_kunci.harga_sewa) {
                salinan_film[j + 1] = salinan_film[j];
                j--;
            }
            salinan_film[j + 1] = film_kunci;
        }
        try {
            int harga_dicari = 0;
            cout << "  Masukkan harga yang ingin dicari (Rp) : ";
            cin >> harga_dicari;
            validasiInputCin();
            validasiMinInt(harga_dicari, 1, "Harga");
            int indeks_hasil = binarySearchHarga(salinan_film, jumlah_film, harga_dicari);
            if (indeks_hasil == -1) throw runtime_error("Film dengan harga Rp" + to_string(harga_dicari) + " tidak ditemukan.");
            cout << "  [v] Film dengan harga Rp" << harga_dicari << " ditemukan!" << endl;
            tampilDetailFilm(salinan_film + indeks_hasil);
            bool ada_lain = false;
            int  kiri     = indeks_hasil - 1;
            int  kanan    = indeks_hasil + 1;
            while (kiri >= 0 && (salinan_film + kiri)->harga_sewa == harga_dicari) {
                if (!ada_lain) { cout << "  Film lain dengan harga sama:" << endl; ada_lain = true; }
                tampilDetailFilm(salinan_film + kiri);
                kiri--;
            }
            while (kanan < jumlah_film && (salinan_film + kanan)->harga_sewa == harga_dicari) {
                if (!ada_lain) { cout << "  Film lain dengan harga sama:" << endl; ada_lain = true; }
                tampilDetailFilm(salinan_film + kanan);
                kanan++;
            }
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
    ...
    } else if (pilihan_aksi == 3) {
        try {
            string genre_dicari;
            cout << "  Genre yang tersedia: Horror, Romance, Action, Thriller, Comedy, Animation" << endl;
            cout << "  Masukkan genre yang dicari : ";
            cin.ignore(); getline(cin, genre_dicari);
            validasiTidakKosong(genre_dicari, "Genre");
            cout << "  Hasil pencarian genre \"" << genre_dicari << "\" :" << endl;
            linearSearchKata(daftar_film, jumlah_film, genre_dicari);
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
        }
        cout << "  [Tekan Enter...]"; cin.get();
...
void menuSewaFilm() {
    ...
    try {
        ...
        validasiTidakKosong(kode_dipilih, "Kode film");

        int indeks_film = cariFilm(kode_dipilih);
        if (indeks_film == -1) throw runtime_error("Film tidak ditemukan.");
        if (!daftar_film[indeks_film].tersedia) throw runtime_error("Film sedang disewa oleh pengguna lain.");

        if (jumlah_sewa >= MAKS_SEWA) throw length_error("Kapasitas data sewa sudah penuh.");
        ...
        int durasi_hari = 0;
        bool durasi_valid = false;
        while (!durasi_valid) {
            try {
                cout << "  Durasi (hari, min 1) : ";
                cin >> durasi_hari;
                validasiInputCin();
                validasiMinInt(durasi_hari, 1, "Durasi");
                durasi_valid = true;
            } catch (exception &e) {
                cout << "  [!] " << e.what() << endl;
            }
    ...
    try {
        ...
        while (!id_valid) {
            try {
                ...
                cin >> id_dikembalikan;
                validasiInputCin();
                validasiMinInt(id_dikembalikan, 1, "ID sewa");
                id_valid = true;
            } catch (exception &e) {
                cout << "  [!] " << e.what() << endl;
            }
        ...
        if (indeks_sewa == -1) throw runtime_error("ID sewa tidak valid atau bukan milik akun ini.");
...
void menuTambahFilm() {
    ...
    try {
        if (jumlah_film >= MAKS_FILM) throw length_error("Kapasitas film sudah penuh.");
        ...
        string kode_baru;
        cout << "  Kode   : "; getline(cin, kode_baru);
        if (kode_baru == "") return;
        validasiTidakKosong(kode_baru, "Kode film");

        for (int i = 0; i < jumlah_film; i++) {
            if (daftar_film[i].kode_film == kode_baru) throw runtime_error("Kode film sudah ada.");
        }

        string judul_baru, genre_baru, studio_baru, negara_baru;
        int    tahun_baru = 0, harga_baru = 0;

        cout << "  Judul  : "; getline(cin, judul_baru);
        validasiTidakKosong(judul_baru, "Judul");

        cout << "  Genre  : "; getline(cin, genre_baru);
        validasiTidakKosong(genre_baru, "Genre");

        bool tahun_valid = false;
        while (!tahun_valid) {
            try {
                cout << "  Tahun  : "; cin >> tahun_baru;
                validasiInputCin();
                validasiMinInt(tahun_baru, 1888, "Tahun");
                tahun_valid = true;
            } catch (exception &e) {
                cout << "  [!] " << e.what() << endl;
            }
        }
        bool harga_valid = false;
        while (!harga_valid) {
            try {
                cout << "  Harga  : "; cin >> harga_baru;
                validasiInputCin();
                validasiMinInt(harga_baru, 1, "Harga");
                harga_valid = true;
            } catch (exception &e) {
                cout << "  [!] " << e.what() << endl;
            }
        ...
        cin.ignore();
        cout << "  Studio : "; getline(cin, studio_baru);
        validasiTidakKosong(studio_baru, "Studio");

        cout << "  Negara : "; getline(cin, negara_baru);
        validasiTidakKosong(negara_baru, "Negara");

        daftar_film[jumlah_film] = {kode_baru, judul_baru, genre_baru, tahun_baru, harga_baru, {studio_baru, negara_baru}, true};
        jumlah_film++;
...
void menuEditFilm() {
    ...
    try {
        ...
        validasiTidakKosong(kode_diedit, "Kode film");

        int indeks_edit = cariFilm(kode_diedit);
        if (indeks_edit == -1) throw runtime_error("Kode film tidak ditemukan.");

        cout << "  [1]Judul  [2]Genre  [3]Tahun  [4]Harga  [5]Studio  [6]Negara" << endl;

        int pilihan_edit = 0;
        bool edit_valid = false;
        while (!edit_valid) {
            try {
                cout << "  Pilihan  : "; cin >> pilihan_edit;
                validasiInputCin();
                validasiRentangInt(pilihan_edit, 1, 6, "Pilihan");
                edit_valid = true;
            } catch (exception &e) {
                cout << "  [!] " << e.what() << endl;
            }
        ...
        if (pilihan_edit == 1) {
            string judul_baru;
            cout << "  Judul baru  : "; getline(cin, judul_baru);
            validasiTidakKosong(judul_baru, "Judul");
            daftar_film[indeks_edit].judul_film = judul_baru;
        } else if (pilihan_edit == 2) {
            string genre_baru;
            cout << "  Genre baru  : "; getline(cin, genre_baru);
            validasiTidakKosong(genre_baru, "Genre");
            daftar_film[indeks_edit].genre_film = genre_baru;
        } else if (pilihan_edit == 3) {
            int tahun_baru = 0;
            bool tahun_valid = false;
            while (!tahun_valid) {
                try {
                    cout << "  Tahun baru  : "; cin >> tahun_baru;
                    validasiInputCin();
                    validasiMinInt(tahun_baru, 1888, "Tahun");
                    tahun_valid = true;
                } catch (exception &e) {
            ...
            daftar_film[indeks_edit].tahun_rilis = tahun_baru;
        } else if (pilihan_edit == 4) {
            int harga_baru = 0;
            bool harga_valid = false;
            while (!harga_valid) {
                try {
                    cout << "  Harga baru  : "; cin >> harga_baru;
                    validasiInputCin();
                    validasiMinInt(harga_baru, 1, "Harga");
                    harga_valid = true;
                } catch (exception &e) {
            ...
            daftar_film[indeks_edit].harga_sewa = harga_baru;
        } else if (pilihan_edit == 5) {
            string studio_baru;
            cout << "  Studio baru : "; getline(cin, studio_baru);
            validasiTidakKosong(studio_baru, "Studio");
            daftar_film[indeks_edit].studio.nama_studio = studio_baru;
        } else if (pilihan_edit == 6) {
            string negara_baru;
            cout << "  Negara baru : "; getline(cin, negara_baru);
            validasiTidakKosong(negara_baru, "Negara");
            daftar_film[indeks_edit].studio.negara_studio = negara_baru;
        }
...
void menuHapusFilm() {
    ...
    try {
        ...
        validasiTidakKosong(kode_dihapus, "Kode film");

        int indeks_hapus = cariFilm(kode_dihapus);
        if (indeks_hapus == -1) throw runtime_error("Kode film tidak ditemukan.");
        if (!daftar_film[indeks_hapus].tersedia) throw runtime_error("Film sedang disewa, tidak bisa dihapus.");
...
void menuSemuaRiwayat() {
    ...
    for (int i = 0; i < jumlah_sewa; i++) {
        string status_sewa  = daftar_sewa[i].sudah_kembali ? "Selesai " : "Aktif   ";
        int    denda_tampil = daftar_sewa[i].sudah_kembali ? daftar_sewa[i].denda : hitungDenda(daftar_sewa[i].harga_per_hari, 0);
...
bool prosesLogin(string &nama_hasil, string &password_hasil, string &role_hasil) {
    ...
    while (sisa_percobaan > 0 && !login_berhasil) {
        try {
            ...
            validasiTidakKosong(input_nama, "Nama");
            validasiTidakKosong(input_password, "Password");
            bool cocok = false;
            ...
            if (!cocok) throw runtime_error("Nama atau password salah.");
            login_berhasil = true;
            ...
            if (sisa_percobaan > 0)
                ...
                cout << endl << "  [!!!] Gagal 3 kali. Kembali ke menu utama." << endl;
        }
    }
...
void prosesRegister(string role_default) {
    try {
        if (jumlah_user >= MAKS_USER) throw length_error("Kapasitas pengguna penuh.");
        cin.ignore();
        string nama_baru, password_baru;
        cout << "  Nama      : "; getline(cin, nama_baru);
        validasiTidakKosong(nama_baru, "Nama");
        for (int i = 0; i < jumlah_user; i++) {
            if (daftar_user[i].nama_pengguna == nama_baru) throw runtime_error("Nama sudah digunakan.");
        }
        cout << "  Password  : "; getline(cin, password_baru);
        validasiTidakKosong(password_baru, "Password");
        daftar_user[jumlah_user] = {nama_baru, password_baru, role_default};
        jumlah_user++;
...
int main() {
    ...
    do {
        ...
        try {
            ...
            cin >> pilihan_awal;
            validasiInputCin();
            validasiRentangInt(pilihan_awal, 0, 2, "Pilihan");
        } catch (exception &e) {
            cout << "  [!] " << e.what() << endl;
            cout << "  [Tekan Enter...]"; cin.ignore(); cin.get();
            continue;
